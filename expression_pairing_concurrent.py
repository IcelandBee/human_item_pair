...
class _RefAttempt:
    ...
    gen_item: ImageItem
    ref_expression: str
    attempted_gen_stems_in_chunk: set[str]
...
def run_pairing(
    ...
    def append_limit_event(
        ref_item: ImageItem,
        ref_expression: str,
        pass_attempt_count_by_ref: dict[str, int],
        limit_logged_ref_stems: set[str],
    ) -> None:
        if ref_item.stem in limit_logged_ref_stems:
            return
        audit.append({
            "event": "ref_pass_attempt_limit_reached",
            "batch_id": config.batch_id,
    ...
    def append_skip_event(ref_item: ImageItem, ref_expression: str, attempted_count: int) -> None:
        audit.append({
            "event": "ref_skipped_after_attempts",
            "batch_id": config.batch_id,
            "seed": config.seed,
    ...
    def append_no_gen_event(ref_item: ImageItem, ref_expression: str) -> None:
        audit.append({
            "event": "no_gen_candidates_left",
            "batch_id": config.batch_id,
            "seed": config.seed,
    ...
    with ThreadPoolExecutor(max_workers=config.workers) as executor:
        while len(results) < config.target_count:
            ...
            def submit_attempt_for_ref(
                ref_item: ImageItem,
                attempted_gen_stems_in_chunk: set[str] | None = None,
            ) -> bool:
                nonlocal processed_gen, attempts
                continuing_chunk = attempted_gen_stems_in_chunk is not None
                chunk_attempts = attempted_gen_stems_in_chunk or set()
                ref_expression = get_expression(ref_item)
                if len(results) >= config.target_count:
                    return False
                if ref_item.stem in active_ref_stems and not continuing_chunk:
                    return False
                if ref_item.stem in accepted_ref_stems_in_pass:
                    return False
                if len(chunk_attempts) >= config.max_ref_attempts_per_gen:
                    return False
                ...
                if remaining_pass_attempts <= 0:
                    append_limit_event(ref_item, ref_expression, pass_attempt_count_by_ref, limit_logged_ref_stems)
                    blocked_ref_stems.add(ref_item.stem)
                    return False
                ...
                gen_candidates = [
                    ...
                    if (
                        gen.stem not in pass_attempted_gen_stems
                        and gen.stem not in reserved_gen_stems
                        and (config.allow_gen_reuse or gen.stem not in processed_unique_gen)
                    )
                ]
                if not gen_candidates:
                    append_no_gen_event(ref_item, ref_expression)
                    blocked_ref_stems.add(ref_item.stem)
                    return False

                rng.shuffle(gen_candidates)
                gen_item = gen_candidates[0]
                pass_attempted_gen_stems.add(gen_item.stem)
                chunk_attempts.add(gen_item.stem)
                pass_attempt_count_by_ref[ref_item.stem] += 1
            ...
            def start_more_refs() -> None:
                while len(active) < config.workers and len(results) + len(active) < config.target_count:
                    ref_item = choose_next_ref(
                        ...
                        ref_usage_count=ref_usage_count,
                        blocked_ref_stems=blocked_ref_stems | accepted_ref_stems_in_pass | active_ref_stems,
                        rng=rng,
                        max_smile_ratio=config.max_smile_ratio,
                        max_big_laugh_ratio=config.max_big_laugh_ratio,
                    )
                    if ref_item is None:
                        return
                    if not submit_attempt_for_ref(ref_item):
                        blocked_ref_stems.add(ref_item.stem)
            ...
            while active and len(results) < config.target_count:
                ...
                for future in done:
                    ...
                    if accepted and ref_item.stem not in accepted_ref_stems_in_pass and len(results) < config.target_count:
                        ref_usage_count[ref_item.stem] += 1
                    ...
                    if (
                        not accepted
                        and ref_item.stem not in accepted_ref_stems_in_pass
                        and pass_attempt_count_by_ref[ref_item.stem] < config.max_ref_attempts_per_pass
                    ):
                        resubmitted_same_ref = submit_attempt_for_ref(
                            ref_item,
                            attempt.attempted_gen_stems_in_chunk,
                        )

                    if not resubmitted_same_ref:
                        completed_refs.add(ref_item.stem)
                    report_progress()

                for ref_stem in completed_refs:
                    if any(active_attempt.ref_item.stem == ref_stem for active_attempt in active.values()):
                        continue
                    active_ref_stems.discard(ref_stem)
                    ref_item = next((ref for ref in all_refs if ref.stem == ref_stem), None)
                    if ref_item is None:
                        continue
                    ref_expression = get_expression(ref_item)
                    ...
                    if ref_stem in accepted_ref_stems_in_pass:
                        continue
                    append_skip_event(
                        ref_item=ref_item,
                        ref_expression=ref_expression,
                        attempted_count=min(
                            config.max_ref_attempts_per_gen,
                            pass_attempt_count_by_ref[ref_stem],
                        ),
                    )
                    if pass_attempt_count_by_ref[ref_stem] >= config.max_ref_attempts_per_pass:
                        append_limit_event(ref_item, ref_expression, pass_attempt_count_by_ref, limit_logged_ref_stems)
                        blocked_ref_stems.add(ref_stem)
                    elif len(pass_attempted_gen_stems_by_ref[ref_stem]) >= len(buckets[ref_item.size_key]["gen"]):
                        blocked_ref_stems.add(ref_stem)
