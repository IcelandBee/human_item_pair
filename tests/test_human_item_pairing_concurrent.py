...
import human_item_pairing_concurrent as human_item_pairing_concurrent_module
...
PROMPT = (
    "Let the person in image 1 hold a mock object shown in image 2 in a realistic "
    "and physically coherent way, preserving object integrity and overall image "
    "consistency, while making only the minimal necessary changes and keeping "
    "everything else in image 1 unchanged."
)
...
def test_run_pairing_uses_workers_for_parallel_human_item_judgements():
...
def test_concurrent_script_is_standalone():
    source = Path("human_item_pairing_concurrent.py").read_text(encoding="utf-8")
...
def test_run_pairing_limits_bad_gen_attempts_per_pass_and_resets_next_pass(monkeypatch):
    ...
    buckets = build_size_buckets(gen_items, ref_items)
    monkeypatch.setattr(
        human_item_pairing_concurrent_module,
        "shuffled_gen_pass",
