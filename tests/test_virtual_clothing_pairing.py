...
def test_run_pairing_limits_bad_gen_attempts_per_pass(monkeypatch):
    gen_items = [make_item("g_bad"), make_item("g_good")]
    ref_items = [make_item(f"r{i}") for i in range(10)]
    buckets = build_size_buckets(gen_items, ref_items)
    monkeypatch.setattr(
        "virtual_clothing_pairing.shuffled_gen_pass",
    ...
    def fake_judge(gen_item, _ref_item):
        ...
        return {
            ...
            "prompt": "Replace the black t-shirt worn by the person in image 1 with the white shirt in image 2, while making minimal changes and preserving the original pose of the person." if accepted else "",
        }
...
def test_run_pairing_allows_only_one_pair_per_gen_per_pass():
    gen_items = [make_item("g_good")]
    ...
    assert len(results) == 2
    assert len([row for row in audit if row["event"] == "pair_accepted"]) == 2
    assert [row["attempt_index_for_gen"] for row in audit if row["event"] == "pair_accepted"] == [1, 1]
...
def test_materialize_pair_outputs_copies_pairs_as_ordered_pngs_and_writes_training_json(tmp_path):
    ...
    ref_path_0 = source_dir / "shirt0.webp"
