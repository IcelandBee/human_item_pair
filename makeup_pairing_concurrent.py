...
- Image 2: the reference makeup image.
- Metadata for image 1.
- Metadata for image 2.

Decide whether this pair is suitable for transferring the makeup style from the person in image 2 to the person in image 1.

Focus only on these suitability points:
1. The target face in image 1 and its key makeup regions must be clear, complete, and unobstructed. Key makeup regions include eyes, eyelids, eyelashes, eyebrows, lips, cheeks, nose bridge, and facial contour.
2. The reference makeup in image 2 must be clearly visible and mainly expressed through face makeup regions, not through clothing, hairstyle, background, or pose.
3. Reject when masks, sunglasses, hands, hair, props, strong shadow, blur, or overexposure make the makeup regions hard to see or unreliable.
...
Extra strict rule for gen_eyes_clear: be stricter for image 1 than image 2. For image 1, set gen_eyes_clear to true only when the target person's iris color is plainly visible and confidently identifiable from an open eye. If the target person is squinting, has narrow eyes, sleepy eyes, half-closed eyelids, downward gaze hiding the iris, eyelids covering much of the iris, or the visible eye opening is too small to confidently identify iris color, set gen_eyes_clear to false. When uncertain, set gen_eyes_clear to false.
...
def get_gender(item: ImageItem) -> str:
    gender = _annotation(item.metadata).get("gender")
    if gender in {"male", "female"}:
        return str(gender)
    return "unknown"


def _target_male_count(total_accepted_after_next: int) -> int:
    return round(total_accepted_after_next * MALE_RATIO / (MALE_RATIO + FEMALE_RATIO))


def choose_next_gen(
    gen_queues: dict[str, list[ImageItem]],
    accepted_gender_counts: dict[str, int],
    rng: random.Random,
) -> ImageItem | None:
    total_accepted = sum(accepted_gender_counts.values())
    next_total = total_accepted + 1
    desired_male = _target_male_count(next_total)
    current_male = accepted_gender_counts.get("male", 0)

    preferred_order = ["male", "female"] if current_male < desired_male else ["female", "male"]
    preferred_order.append("unknown")

    for gender in preferred_order:
        queue = gen_queues.get(gender, [])
        if queue:
            return queue.pop(0)

    non_empty = [gender for gender, queue in gen_queues.items() if queue]
    if not non_empty:
        return None
    gender = rng.choice(non_empty)
    return gen_queues[gender].pop(0)
...
def build_user_text(gen_item: ImageItem, ref_item: ImageItem) -> str:
    ...
    return (
        "Judge whether image 1 and image 2 form a suitable makeup transfer pair. "
...
def infer_pair_decision(
    ...
    messages = [
        {"role": "system", "content": MAKEUP_SYSTEM_PROMPT},
        {
            "role": "user",
            "content": [
...
def get_output_dimensions(gen_item: ImageItem) -> dict[str, int]:
    ...
    if "x" in gen_item.size_key:
        raw_width, raw_height = gen_item.size_key.split("x", 1)
...
def _build_gen_queues(
    ...
    queues = {"male": [], "female": [], "unknown": []}
    for item in gen_items:
        queues[get_gender(item)].append(item)
    for queue in queues.values():
        rng.shuffle(queue)
    return queues


def _gender_balanced_gen_pass(
    gen_items: list[ImageItem],
    accepted_gender_counts: dict[str, int],
    rng: random.Random,
) -> list[ImageItem]:
    gen_queues = _build_gen_queues(gen_items, rng)
    planned_counts = dict(accepted_gender_counts)
    result: list[ImageItem] = []
    while True:
        gen_item = choose_next_gen(gen_queues, planned_counts, rng)
        if gen_item is None:
            return result
        result.append(gen_item)
        planned_counts[get_gender(gen_item)] = planned_counts.get(get_gender(gen_item), 0) + 1
...
def _run_pairing_single_worker(
    ...
    while len(results) < config.target_count:
        gen_pass = _gender_balanced_gen_pass(gen_pool, accepted_gender_counts, rng)
        accepted_in_pass = 0
...
def build_output_paths(output_dir: Path, batch_id: str) -> tuple[Path, Path]:
    return (
        output_dir / f"makeup_{batch_id}.json",
        output_dir / f"makeup_{batch_id}.audit.jsonl",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build VLM-judged makeup transfer pairings.")
